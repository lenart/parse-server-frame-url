  ...
  _createClass(UserController, [{
    ...
    value: function sendVerificationEmail(user) {
      ...
      this.getUserIfNeeded(user).then(function (user) {
        ...
        var link = buildVerificationLink(_this3.config.verifyEmailURL, username, token, _this3.config);
        var options = {
    ...
    value: function sendPasswordResetEmail(email) {
      ...
      return this.setPasswordResetToken(email).then(function (user) {
        ...
        var link = buildVerificationLink(_this4.config.requestResetPasswordURL, username, token, _this4.config);
        var options = {
...
function buildVerificationLink(destination, username, token, config) {
  var usernameAndToken = 'token=' + token + '&username=' + username;

  if (this.config.parseFrameURL) {
    var destinationWithoutHost = destination.replace(this.config.publicServerURL, '');
