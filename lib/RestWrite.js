...
RestWrite.prototype.transformUser = function () {
  ...
  return promise.then(function () {
    ...
    if (_this7.config.passwordPolicy) {
      var _ret2 = function () {
        var policyError = 'Password does not meet the Password Policy requirements.';

        // check whether the password conforms to the policy
        if (_this7.config.passwordPolicy.patternValidator && !_this7.config.passwordPolicy.patternValidator(_this7.data.password) || _this7.config.passwordPolicy.validatorCallback && !_this7.config.passwordPolicy.validatorCallback(_this7.data.password)) {
          return {
            v: Promise.reject(new Parse.Error(Parse.Error.VALIDATION_ERROR, policyError))
          };
        }

        // check whether password contain username
        if (_this7.config.passwordPolicy.doNotAllowUsername === true) {
          if (_this7.data.username) {
            // username is not passed during password reset
            if (_this7.data.password.indexOf(_this7.data.username) >= 0) return {
                v: Promise.reject(new Parse.Error(Parse.Error.VALIDATION_ERROR, policyError))
              };
          } else {
            // retrieve the User object using objectId during password reset
            defer = _this7.config.database.find('_User', { objectId: _this7.objectId() }).then(function (results) {
              if (results.length != 1) {
                throw undefined;
              }
              if (_this7.data.password.indexOf(results[0].username) >= 0) return Promise.reject(new Parse.Error(Parse.Error.VALIDATION_ERROR, policyError));
              return Promise.resolve();
            });
          }
        }
...
RestWrite.prototype.handleInstallation = function () {
  ...
  promise = promise.then(function () {
    ...
    if (!_this9.query && !_this9.data.deviceType && !idMatch) {
...
RestWrite.prototype.runDatabaseOperation = function () {
  ...
  if (this.query) {
    ...
    return this.config.database.update(this.className, this.query, this.data, this.runOptions).then(function (response) {
      ...
      _this10._updateResponseWithData(response, _this10.data);
