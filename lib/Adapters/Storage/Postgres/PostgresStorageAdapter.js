  ...
  var _loop = function _loop(fieldName) {
    ...
    if (fieldName.indexOf('.') >= 0) {
      ...
      (function () {
        ...
        var clauses = [];
        var clauseValues = [];
        fieldValue.forEach(function (subQuery, idx) {
...
var PostgresStorageAdapter = exports.PostgresStorageAdapter = function () {
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function createObject(className, schema, object) {
      ...
      Object.keys(object).forEach(function (fieldName) {
        ...
        if (!schema.fields[fieldName] && className === '_User') {
          if (fieldName === '_email_verify_token' || fieldName === '_failed_login_count' || fieldName === '_perishable_token') {
      ...
      var geoPointsInjects = Object.keys(geoPoints).map(function (key, idx) {
      ...
      var _loop2 = function _loop2(_fieldName) {
        ...
        if (fieldValue === null) {
          ...
          (function () {
            ...
            index += 1;
            values.push(_fieldName);
            var update = Object.keys(fieldValue).reduce(function (lastKey, key) {
          ...
          return {
            v: Promise.reject(new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, 'Postgres doesn\'t support update ' + JSON.stringify(fieldValue) + ' yet'))
          };
        }
      };

      for (var _fieldName in update) {
        var _ret5 = _loop2(_fieldName);
    ...
    value: function performInitialization(_ref4) {
      ...
      promises = promises.concat([this._client.any(json_object_set_key).catch(function (err) {
        console.error(err);
      }), this._client.any(array_add).catch(function (err) {
        console.error(err);
      }), this._client.any(array_add_unique).catch(function (err) {
        console.error(err);
      }), this._client.any(array_remove).catch(function (err) {
        console.error(err);
      }), this._client.any(array_contains_all).catch(function (err) {
        console.error(err);
      }), this._client.any(array_contains).catch(function (err) {
        console.error(err);
      })]);
...
var json_object_set_key = 'CREATE OR REPLACE FUNCTION "json_object_set_key"(\
  "json"          jsonb,\
  "key_to_set"    TEXT,\
